...
-- Indexes (SAFE TO RUN IDEMPOTENTLY)
create index if not exists idx_novels_user on public.novels(user_id);
create index if not exists idx_acts_novel on public.acts(novel_id);
create index if not exists idx_chapters_act on public.chapters(act_id);
create index if not exists idx_scenes_chapter on public.scenes(chapter_id);
create index if not exists idx_codex_novel on public.codex(novel_id);
...
-- DROP first to avoid duplication errors if running multiple times without full clean
drop policy if exists "Users can allow select their own novels" on public.novels;
drop policy if exists "Users can allow insert their own novels" on public.novels;
drop policy if exists "Users can allow update their own novels" on public.novels;
drop policy if exists "Users can allow delete their own novels" on public.novels;
...
drop policy if exists "Users can interact with their own acts" on public.acts;
create policy "Users can interact with their own acts" on public.acts using (auth.uid() = user_id);

drop policy if exists "Users can interact with their own chapters" on public.chapters;
create policy "Users can interact with their own chapters" on public.chapters using (auth.uid() = user_id);

drop policy if exists "Users can interact with their own scenes" on public.scenes;
create policy "Users can interact with their own scenes" on public.scenes using (auth.uid() = user_id);

drop policy if exists "Users can interact with their own codex" on public.codex;
